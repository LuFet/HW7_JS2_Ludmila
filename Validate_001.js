...
function init() {
    ...
    form1.confirm_password.onchange = confirm_passOnChange;
...
function passOnChange() {
    ...
    if(document.getElementById('confirm_password').value==null ||
        ...
        if(confirm_passOnChange)
        document.getElementById(msgId).innerHTML = "x";
        this.className = "valid";
    }
  }
function confirm_passOnChange() {
    let pattern = this.dataset.val,
    msg = this.dataset.valMsg,
        msgId = this.dataset.valMsgId,
        value = this.value;
...
function validateForm() {
    ...
    if (invalid) {
        document.getElementById(msgId).innerHTML = "Допущены ошибки при заполнении формы.";
